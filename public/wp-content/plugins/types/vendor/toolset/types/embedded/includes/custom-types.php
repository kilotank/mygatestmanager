...
function wpcf_custom_types_register( $post_type, $data ) {
	...
	if( isset( $data['editor'] ) && $data['editor'] == 'block' ) {
		// block editor chosen
		// make sure rest is active
		$data['show_in_rest'] = true;
	} else {
		// classic chosen or the post type was created pre gutenberg
		// (two filters because it's different for inbuild block editor and gutenberg plugin)
		$to_be_registered_post_type = $post_type;
		foreach( array( 'gutenberg_can_edit_post_type', 'use_block_editor_for_post_type' ) as $filter ) {
			add_filter( $filter, function( $use_block_editor, $post_type ) use ( $to_be_registered_post_type ) {
				if ( $post_type == $to_be_registered_post_type ) {
					return false;
				}
